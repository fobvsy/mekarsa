<?php
/**
 * Mekarsa Coffee Bar - Admin Kelola Kategori Produk
 * CRUD kategori produk: tambah, edit, hapus
 */
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
        ...
        // Cek duplicate name
        if (empty($formErrors)) {
            $check = $pdo->prepare("SELECT id FROM product_categories WHERE name = ?");
            $check->execute([$name]);
            if ($check->fetch()) {
        ...
        if (empty($formErrors)) {
            ...
            header('Location: product-categories.php?msg=added');
            exit;
        } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
        ...
        // Cek duplicate name (exclude current ID)
        if (empty($formErrors)) {
            $check = $pdo->prepare("SELECT id FROM product_categories WHERE name = ? AND id != ?");
        ...
        if (empty($formErrors)) {
            $stmt = $pdo->prepare("UPDATE product_categories SET name=?, description=?, updated_at=NOW() WHERE id=?");
            $stmt->execute([$name, $description, $id]);
            header('Location: product-categories.php?msg=updated');
            exit;
        } else {
            $flashType = 'error';
            $flashMsg  = 'Terdapat kesalahan pada form. Periksa kembali.';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
        ...
        if ($id > 0) {
            ...
            if ($count > 0) {
                header('Location: product-categories.php?msg=error_in_use');
                exit;
            } else {
                $pdo->prepare("DELETE FROM product_categories WHERE id = ?")->execute([$id]);
                header('Location: product-categories.php?msg=deleted');
                exit;
            }
        }
        header('Location: product-categories.php?msg=error');
        exit;
    }
    ...
    $msgMap = [
        ...
        'error_in_use' => ['Gagal menghapus. Masih ada produk di dalam kategori ini.', 'error'],
    ...
    // Query list dengan count produk
    ...
    // Total categories
    $totalCatStmt = $pdo->query("SELECT COUNT(*) FROM product_categories");
...
<head>
    ...
    <title>Kategori Produk — Mekarsa Admin</title>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ===== SIDEBAR (Simplified for CSS space, assume identical structure) ===== */
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="product-categories.php" class="nav-item active">
...
<div class="main">
    ...
    <header class="topbar">
        ...
        <div class="topbar-right">
            ...
            <a href="../menu.php" class="topbar-view-site" target="_blank">
    ...
    <main class="page-content">
        ...
        <div class="page-header">
            <div class="page-header-title">
                <h1><i class="fas fa-tags" style="color:var(--orange);margin-right:0.5rem;font-size:1.4rem;"></i>Kategori Produk</h1>
                <p>Kelola klasifikasi atau jenis-jenis menu coffee yang tersedia.</p>
            </div>
        ...
        <div class="top-widgets">
            ...
            <div class="filter-bar">
                <form method="GET" action="product-categories.php">
                    ...
                    <?php if ($search): ?>
                        <a href="product-categories.php" class="btn-secondary" style="border:none;background:transparent;"><i class="fas fa-xmark"></i> Reset</a>
                    <?php endif; ?>
                </form>
            </div>
        </div>
            ...
            <div class="table-wrapper">
                    ...
                    <table class="data-table">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 150px; text-align: center;">Jumlah Produk</th>
                            ...
                            <?php foreach ($categories as $i => $cat): ?>
                                <tr>
                                    ...
                                    <td style="text-align: center;">
                                        <span class="badge <?= $cat['product_count'] == 0 ? 'badge--zero' : '' ?>">
                                            <?= $cat['product_count'] ?>
                                        </span>
                                    </td>
                                    <td>
                                        <div class="action-group" style="justify-content: center;">
                                            ...
                                            <button class="action-btn action-btn--danger" onclick="confirmDelete(<?= $cat['id'] ?>, '<?= addslashes(htmlspecialchars($cat['name'])) ?>', <?= $cat['product_count'] ?>)" title="Hapus">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p><?= $search ? 'Tidak ada kategori yang cocok dengan pencarian.' : 'Silakan tambahkan kategori untuk mengelompokkan produk Anda.' ?></p>
                        <?php if (!$search): ?>
...
<div class="modal-overlay" id="catModal">
    <div class="modal">
        ...
        <form id="catForm" method="POST" action="product-categories.php" novalidate>
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-input <?= isset($formErrors['name']) ? 'is-error' : '' ?>"
                           placeholder="Contoh: Signature Drinks" required
    ...
    <div class="modal confirm-modal">
        ...
        <div class="modal-body">
            ...
            <div id="deleteWarning" style="display:none; background:rgba(248,113,113,0.1); border:1px solid rgba(248,113,113,0.3); color:var(--red); padding:0.75rem; border-radius:8px; font-size:0.85rem; margin-bottom:1.5rem;">
                <i class="fas fa-triangle-exclamation"></i> Kategori ini tidak bisa dihapus karena masih memiliki <strong id="deleteCount"></strong> produk.
            </div>
